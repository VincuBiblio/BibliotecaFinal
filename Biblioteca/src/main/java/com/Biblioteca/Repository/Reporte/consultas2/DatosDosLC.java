...
public interface DatosDosLC extends JpaRepository<PrestamoLibroCliente, Long> {
...
@Query(value = "select p.cedula,p.nombres, p.apellidos, p.telefono, p.email, " +
        ...
        "and precli.id_cliente NOT IN (SELECT id_cliente FROM copias_cliente where mes= :mes and anio= :anio and dia=precli.dia_prestamo) ", nativeQuery = true)
