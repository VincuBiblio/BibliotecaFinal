...
public interface DatosTresLCCopia extends JpaRepository<PrestamoLibroCliente, Long> {
    ...
    @Query(value = "select p.cedula,p.nombres, p.apellidos, p.telefono, p.email, " +
            ...
            "and precli.id_cliente NOT IN (SELECT id_cliente FROM servicios_varios_cliente where mes_prestamo=:mes and anio_prestamo=:anio and dia_prestamo=precli.dia_prestamo) " +
            "and precli.id_cliente IN (SELECT id_cliente FROM computo_cliente where mes= :mes and anio= :anio and dia=precli.dia_prestamo) " +
            "and precli.id_cliente IN (SELECT id_cliente FROM copias_cliente where mes= :mes and anio= :anio and dia=precli.dia_prestamo) " , nativeQuery = true)
