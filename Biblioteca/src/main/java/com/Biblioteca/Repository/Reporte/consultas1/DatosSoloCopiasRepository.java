...
public interface DatosSoloCopiasRepository extends JpaRepository<CopiasCliente, Long> {
    ...
    @Query(value = "select p.cedula,p.nombres, p.apellidos, p.telefono, p.email," +
            ...
            "and cocli.id_cliente NOT IN (SELECT id_cliente FROM computo_cliente where mes= :mes and anio= :anio and dia=cocli.dia) " +
            "and cocli.id_cliente NOT IN (SELECT id_cliente FROM prestamolibros_cliente where mes_prestamo= :mes and anio_prestamo= :anio and dia_prestamo=cocli.dia) " +
            "and cocli.id_cliente NOT IN (SELECT id_cliente FROM servicios_varios_cliente where mes_prestamo= :mes and anio_prestamo= :anio  and dia_prestamo=cocli.dia)", nativeQuery = true)
