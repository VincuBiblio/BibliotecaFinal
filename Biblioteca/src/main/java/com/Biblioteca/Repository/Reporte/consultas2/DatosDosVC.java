...
public interface DatosDosVC  extends JpaRepository<ServiciosVariosCliente, Long> {
    ...
    @Query(value = "select p.cedula,p.nombres, p.apellidos, p.telefono, p.email, " +
            ...
            "and svc.id_cliente IN (SELECT id_cliente FROM computo_cliente where mes=:mes and anio=:anio and dia=svc.dia_prestamo) " +
            "and svc.id_cliente NOT IN (SELECT id_cliente FROM copias_cliente where mes=:mes and anio= :anio and dia=svc.dia_prestamo)" +
